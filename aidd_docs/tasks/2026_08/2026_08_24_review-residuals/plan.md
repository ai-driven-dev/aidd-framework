...
| Decision | Why |
| --- | --- |
| A partial record is superseded at read time, never corrected in place | The sink is append-only by design. Correcting a stored line would make the file a mutable database and break the one property every consumer relies on. A later, strictly more complete reading is therefore a second line that wins, never an edit. |
| Superseding a re-read and collapsing two routes stay two mechanisms | They look alike and are not. Checked before planning: only the Claude Code reader supplies `billed_request_id`, so the existing collapse never reaches Codex — the tool this defect is about. And on the export route `turn_id` is a prompt id shared by several billed calls, so the key that works for a re-read would merge distinct calls there. One reconciles two routes seeing one call; the other reconciles two readings of one source. |
