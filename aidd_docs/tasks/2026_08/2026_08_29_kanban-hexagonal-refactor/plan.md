...
| **Scope**  | Phases 1-6 are the hexagonal-refactor pass. Phase 7 (`interactive --live`) is the deferred "second temps" improvement, run only after 1-6 land. |
...
| 5   | Frontend renders the server board              | [`phase-5.md`](./phase-5.md) |
| 6   | CLI boundary hardened around the entrypoint     | [`phase-6.md`](./phase-6.md) |
| 7   | `interactive --live` opt-in watcher refresh     | [`phase-7.md`](./phase-7.md) |
...
| Live refresh in the ink view is opt-in behind `interactive --live` (phase 7), not the default; `web` stays always-live | The current ink view is fetch-once; making it always-live is a behaviour change. The `TaskDocumentWatcher` port and `createKanbanRuntime.createWatcher` factory are shared by `web` and the future `--live` flag. |
