...
| OpenCode | `[v]` aucun — SDK OpenTelemetry embarqué mais **aucun exportateur**, et une session réelle avec `OTEL_EXPORTER_OTLP_ENDPOINT` posé n'a rien émis | `[?]` hors export | sans objet | `[?]` hors export | `[v]` non — aucun des événements de plugin listés n'expose d'usage |
