...
export interface TelemetryEndpointClearOptions {
  readonly projectRoot: string;
}

export interface TelemetryEndpointClearResult {
...
/** Undoes exactly what `aidd telemetry endpoint <url>` wrote, through the same
 * `removeEntriesFromJson` `aidd clean` uses, never a second remover. Which section of
 * which file to clean comes from that tool's `capabilities.telemetry`. Ported unchanged
 * from `TelemetryOffUseCase`: `off` shrank to the switch alone, and an export
 * configuration nobody asked `off` to touch must survive it. */
export class TelemetryEndpointClearUseCase {
  constructor(
  ...
  async execute(options: TelemetryEndpointClearOptions): Promise<TelemetryEndpointClearResult> {
    ...
    return { removedFiles, manualUnsetReminders };
  }
  ...
  private async cleanEntry(
    ...
    if (!(await this.fs.fileExists(fullPath))) {
      ...
      // resolution that can't succeed on every future `clear` helps nobody), but this must
      // never be silent.
      this.logger.warn(
