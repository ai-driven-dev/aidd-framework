...
// Side-effect imports: TelemetryEndpointClearUseCase resolves each tool's telemetry story
...
const HOME_SETTINGS_PATH = join(HOME_DIR, ".claude", "settings.json");
...
function buildUseCase(manifest: Manifest | null = null, seed: Record<string, string> = {}) {
  ...
  const useCase = new TelemetryEndpointClearUseCase(fs, manifestRepo, logger);
  return { fs, manifestRepo, logger, useCase };
}

describe("TelemetryEndpointClearUseCase — nothing tracked", () => {
  it("succeeds and changes nothing when no manifest exists", async () => {
...
describe("TelemetryEndpointClearUseCase — undoing what the manifest recorded", () => {
...
describe("TelemetryEndpointClearUseCase — the inherited --scope user caveat", () => {
  function manifestWithUserScopeEntry(): Manifest {
    ...
    manifest.updateToolMergeFiles("claude", [
      {
        // Recorded exactly as EnableToolTelemetryUseCase records --scope user: a
  ...
  it("resolves and cleans the traversal path when the project hasn't moved", async () => {
    ...
    const { fs, useCase } = buildUseCase(manifestWithUserScopeEntry(), {
      [HOME_SETTINGS_PATH]: before,
    });

    const result = await useCase.execute({ projectRoot: PROJECT_ROOT });
    expect(result.removedFiles).toEqual([HOME_SETTINGS_PATH]);
    const after = JSON.parse(fs.getFile(HOME_SETTINGS_PATH) as string);
  ...
  it("warns instead of silently forgetting when the traversal resolves nowhere — the project-moved case", async () => {
    ...
    expect(fs.listAll()).toHaveLength(0);
    expect(
      logger.warnMessages.some((m) => m.includes(HOME_SETTINGS_PATH) && m.includes("moved"))
    ).toBe(true);
...
describe("TelemetryEndpointClearUseCase — manual-unset reminders", () => {
