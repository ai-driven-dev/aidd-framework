...
// Side-effect imports: TelemetryEndpointUseCase now resolves each tool's telemetry story
// from the registry, so every AI tool must be registered for these tests to see it.
...
function buildUseCase(manifest: Manifest | null = null) {
  ...
  const enableTool = new EnableToolTelemetryUseCase(fs, hasher, manifestRepo, logger);
  const useCase = new TelemetryEndpointUseCase(
    manifestRepo,
    enableTool,
    logger,
...
function baseOptions(overrides: Partial<Parameters<TelemetryEndpointUseCase["execute"]>[0]> = {}) {
  return {
...
describe("TelemetryEndpointUseCase — the switch is never its business", () => {
  it("still works and says so, reporting claude as not installed, with no manifest at all", async () => {
...
describe("TelemetryEndpointUseCase — per-tool honesty", () => {
  ...
  it("reports copilot's environment variable rather than claiming to have set it", async () => {
    ...
    // Nothing new on disk for copilot beyond claude's settings file — copilot's "config" is
    // a variable the user exports themselves.
...
describe("TelemetryEndpointUseCase — scope", () => {
...
describe("TelemetryEndpointUseCase — say the file before touching it", () => {
...
describe("TelemetryEndpointUseCase — endpoint validation", () => {
  it("rejects a blank endpoint and writes nothing", async () => {
    const { fs, useCase } = buildUseCase(claudeManifest());
    await expect(useCase.execute(baseOptions({ endpoint: "  " }))).rejects.toThrow(/no default/i);
    expect(fs.listAll()).toHaveLength(0);
  });
  ...
  it("warns, but still configures, a remote endpoint — it was typed, not inherited", async () => {
  ...
  it("says nothing extra for a loopback endpoint", async () => {
    const { useCase, logger } = buildUseCase(claudeManifest());
    await useCase.execute(baseOptions({ endpoint: "http://127.0.0.1:4318" }));
