...
Write a Markdown Epic to `aidd_docs/backlog/epics/<slug>.md`.
Use `goal` for Product Brief or product-goal alignment. Use `source` for a different origin; never store the same reference in both.

Stories and Tasks identify their Epic through `parent`; Spikes identify blocked artifacts through `parents`. Do not copy inverse links into the Epic.
