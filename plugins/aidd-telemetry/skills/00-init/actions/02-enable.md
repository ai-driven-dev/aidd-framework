...
3. **Turn it on.** Run `node <telemetry-switch.js> on`, which merges into whatever the config already holds. The same run also adds `aidd_docs/runs/` to `.gitignore` — the journal belongs to this repository and is never offered to a commit.
...
6. **Say it is reversible, and what reversing keeps.** `node <telemetry-switch.js> off` stops the recording from that moment; sessions already measured stay measured and still report.
