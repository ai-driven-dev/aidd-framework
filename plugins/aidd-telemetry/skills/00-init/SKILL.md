...
- What history keeps is relayed exactly as `aidd telemetry forget` states it — a journal tracked by git right now as certainly held, one not tracked as possibly held — and is never presented as removable. No command in this skill rewrites git history.
